...
if ($_SESSION['userLogin'] == 'loggato') {
	...
	if (isset($_REQUEST['invia'])) {
		$date = $_POST['date'];
		$query = mysqli_query($connetion, "SELECT timestamp, status, battery, speed, coordsx, coordsy FROM rover WHERE CAST(timestamp as DATE) = '$date' AND roverID=2 ORDER BY id DESC");
		...
		$query = mysqli_query($connetion, "SELECT * FROM rover WHERE roverID=2 ORDER BY id DESC");
